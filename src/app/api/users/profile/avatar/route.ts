...
export async function PATCH(req: Request) {
    ...
    try {
        ...
        console.log(`[Proxy] Updating Avatar for user: ${email} (current id: ${userId})`);
        ...
        if (!token) {
            console.error("[Proxy Profile/Avatar] Access token MISSING from session.");
            return NextResponse.json({ error: "Access token missing. Please log out and back in." }, { status: 401 });
        }

        const response = await fetch("http://localhost:8083/api/users/profile/avatar", {
            method: "PATCH",
            headers: {
        ...
        if (!response.ok) {
            ...
            console.error(`[Proxy Profile/Avatar] Backend Error ${response.status}: ${errorText}`);
